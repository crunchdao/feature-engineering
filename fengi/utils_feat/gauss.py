...
def _update_x(x: Union[np.ndarray[Any, Any], List[Any]]) -> np.ndarray[Any, Any]:
    x = np.asarray(x)
    if len(x.shape) == 1:
        x = x[:, np.newaxis]
    elif len(x.shape) != 2:
...
class Gaussianize(sklearn.base.TransformerMixin):
    ...
    def __init__(
        ...
        self.coefs__: List[float] = []  # Store tau for each transformed variable
        self.verbose = verbose

    def fit(self, x: np.ndarray[Any, Any], y: Optional[Any]) -> "Gaussianize":
